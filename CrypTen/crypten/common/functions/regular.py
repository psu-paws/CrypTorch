...
def sum(self, *args, **kwargs):
    ...
    if "dtype" in kwargs:
        if kwargs["dtype"] != rs.get_ring_dtype():
            warnings.warn(f"Unexpected dtype {kwargs['dtype']} explicitly passed, expected ring element dtype is {rs.get_ring_dtype()}")
        result._tensor = self._tensor.sum(*args, **kwargs)
    else:
        result._tensor = self._tensor.sum(*args, **kwargs, dtype=rs.get_ring_dtype())
