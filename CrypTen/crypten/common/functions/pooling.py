...
def max_pool2d(
    ...
    # dynamically reduce pad value abs to prevent overflows.
    compare_msb = min(cfg.get_with_default("functions.compare_msb", rs.get_ring_size() // 2), rs.get_ring_size())
    print(f"{compare_msb=}")
    pad_value = -(2 ** (compare_msb - 3))
    print(f"{pad_value=}")
    ...
    # print(f"MAX POOL A:{self.get_plain_text()=}")
    # print(f"MAX POOL B:{max_input.get_plain_text()=}")
    # print(f"MAX POOL C:{max_input.share=}")
    # print(f"MAX POOL D:{max_input.encoder.scale=}")
    
    crypten_maxpool = (cfg.get_with_default("functions.maxpool", "fixed") == "crypten")
    if return_indices or crypten_maxpool:
...
def avg_pool2d(
    ...
    sum_input = self.clone()
    sum_input.data, output_size = _pool2d_reshape(
        self.data,
        kernel_size,
        padding=padding,
        stride=stride,
    ...
    sum_vals = sum_input.sum(dim=-1)
    sum_vals = sum_vals.view(output_size)
    return sum_vals * (1 / divisor)
...
def adaptive_avg_pool2d(self, output_size):
    ...
    # print(f"{resized_input.get_plain_text()=}")
    # print(f"{resized_input.get_plain_text().mean()=}")
    # print(f"{resized_input.size()=}")
    # print(f"{args=}")
    # print(f"{kwargs=}")
