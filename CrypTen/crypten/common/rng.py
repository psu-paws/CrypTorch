...
def generate_random_ring_element(size, generator=None, **kwargs):
    _ring_dtype = rs.get_ring_dtype()
#def generate_random_ring_element(size, ring_size=(2**64), generator=None, ring_dtype="long", **kwargs):
    ...
    ring_size = 2 ** (torch.iinfo(_ring_dtype).bits)
    ...
    rand_element = rand_element.to(_ring_dtype)
...
def generate_kbit_random_tensor(size, bitlength=None, generator=None, **kwargs):
    ...
    if bitlength is None:
        bitlength = torch.iinfo(_ring_dtype).bits
    #if bitlength == 64:
    if bitlength == torch.iinfo(_ring_dtype).bits:
    ...
    rand_tensor = torch.randint(
        0, 2**bitlength, size, generator=generator, dtype=_ring_dtype, **kwargs
    )
