...
import crypten.common.tensor_types
...
def where(condition, input, other):
    ...
    if is_encrypted_tensor(condition):
        ...
        if cfg.get_with_default("functions.where", "fixed") == "crypten":
            print("Old Where")
            return input * condition + other * (1 - condition)
        else:
            return other + condition * (input - other)
    elif torch.is_tensor(condition):
...
class LogOperation:
    ...
    def __enter__(self):
        stats = crypten.communicator.get().get_communication_stats()
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback):
        stats = crypten.communicator.get().get_communication_stats()
        rounds = stats["rounds"] - self.rounds
        bytes = stats["bytes"] - self.bytes
        time = stats["time"] - self.time
        
        print(f"{self.name}: {rounds} rounds, {bytes} bytes, {time} seconds")
