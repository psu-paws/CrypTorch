...
class FixedPointEncoder:
    ...
    def __init__(self, precision_bits=None):
        if precision_bits is None:
            ...
            # We should not be using CrypTen's cfg's precision
            assert(False)
    ...
    def encode(self, x, device=None):
        ...
        if isinstance(x, CrypTensor):
            ...
            return torch.tensor(
                [self._scale * x], dtype=_ring_dtype, device=device
            ).squeeze()
        elif isinstance(x, list):
            return (
                (torch.tensor(x, dtype=torch.float, device=device).mul_(self._scale)).to(_ring_dtype)
            )
        elif is_float_tensor(x):
            return (self._scale * x).to(_ring_dtype)
        # For integer types cast to long prior to scaling to avoid overflow.
        elif is_int_tensor(x):
            return self._scale * x.to(_ring_dtype)
        elif isinstance(x, np.ndarray):
            return (self._scale * torch.from_numpy(x)).to(_ring_dtype).to(device)
