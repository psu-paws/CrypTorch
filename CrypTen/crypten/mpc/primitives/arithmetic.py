...
class ArithmeticSharedTensor:
    ...
    def _arithmetic_function(self, y, op, inplace=False, *args, **kwargs):  # noqa:C901
        ...
        # print(f"BBBBBB {y=}")
        is_float = not private and (isinstance(y, float) or is_float_tensor(y))
    ...
    def div_(self, y):
        ...
        if isinstance(y, int) or is_int_tensor(y):
            ...
            if comm.get().get_world_size() > 2:
                ...
                # print(f"POST {y=}")
            elif cfg.mpc.truncation == "exact-fake":
                plain_text = self.get_plain_text()
                # print(f"{plain_text.max()=}")
                # print(f"{plain_text.min()=}")
                # print(f"{plain_text.mean()=}")
            ...
            elif cfg.mpc.truncation == "exact" and isinstance(y, int) and y.bit_count() == 1:
                from crypten.mpc.primitives.converters import _A2B, _B2A
                ...
                result.share >>= shift
                # print(f"{result.get_plain_text()=}")
                result = _B2A(result, precision=precision)
                # print(f"{result.get_plain_text()=}")
                self.share = result.share
                # print(f"{self.get_plain_text()=}")
            elif cfg.mpc.truncation == "rsst" and isinstance(y, int) and y.bit_count() == 1: 
                protocol = globals()[cfg.mpc.protocol]
                protocol.truncate_reduced_slack(self, y)
            else:
    ...
    #     self_plaintext = self.get_plain_text()
        
    #     print(f"{self_plaintext=}")
    #     print(f"{self.size()=}")
    #     print(f"{self_plaintext.mean()=}")
        
    #     z = self._sum_pool2d(
    #         kernel_size, stride=stride, padding=padding, ceil_mode=ceil_mode
    #     )
        
    #     print(f"{z.get_plain_text()=}")
    #     print(f"{z.get_plain_text().mean()=}")
    #     print(f"{z.size()=}")
    ...
    #     print(f"{type(self.share)=}")
    #     print(f"{self.share.dtype=}")
    #     print(f"{self.share=}")
    #     print(f"{self.share.sum()=}")
    ...
    #     print(f"{result_plain_text=}")
    #     print(f"{result_plain_text.size()=}")
    #     print(f"{result_plain_text.mean()=}")
    #     print(f"{result.share.dtype=}")
    #     print(f"{result.share=}")
    #     print(f"{result.share.sum()=}")
    #     return result
