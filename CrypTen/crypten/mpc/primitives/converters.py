...
def _A2B(arithmetic_tensor):
    ...
    #print("333", binary_tensor.share.dtype)
    # return the result:
...
def _B2A(binary_tensor, precision=None, bits=None):
    if bits is None:
        bits = torch.iinfo(binary_tensor.share.dtype).bits
    ...
    if bits == 1:
        ...
        multiplier = torch.cat(
            [
                torch.tensor([1], dtype=binary_tensor.share.dtype, device=binary_tensor.device) << i
                for i in range(bits)
            ]
        )
        ...
        arithmetic_tensor = arithmetic_bits.mul_(multiplier).sum(0, dtype=binary_tensor.share.dtype)
