...
def range_decorator(msg: str):
    def range_decorator_instance(f):
    ...
    return range_decorator_instance

class TupleProviderMeta(type):
    ...
    def __new__(cls, clsname, bases, attrs):
        ...
        for method in TRACEABLE_FUNCTIONS:
            if method in attrs:
                attrs[method] = range_decorator(f"PROVIDER-{method}")(attrs[method])
        return type.__new__(cls, clsname, bases, attrs)

class TupleProvider(metaclass=TupleProviderMeta):
    TRACEABLE_FUNCTIONS = [
        "generate_additive_triple",
        "square",
        "generate_binary_triple",
    ...
    def RSST_rng(self, size, t, device=None):
        raise NotImplementedError("TupleProvider RSST_rng not implemented")
