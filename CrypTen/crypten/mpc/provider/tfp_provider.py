...
class TrustedFirstParty(TupleProvider):
    ...
    def RSST_rng(self, size, t, device=None):
        r = generate_random_ring_element(size, device=device)
        rMSB = ArithmeticSharedTensor(r >> (rs.get_ring_size() - 1) & 1, precision=0, src=0)
        rUpper = (r & ((2 ** (rs.get_ring_size() - 1)) - 1)) >> t
        rUpper = ArithmeticSharedTensor(rUpper, precision=0, src=0)
        r = ArithmeticSharedTensor(r, precision=0, src=0)
        return r, rMSB, rUpper