...
class MPCTensor(CrypTensor):
    ...
    def from_shares(share, precision=None, ptype=Ptype.arithmetic):
        result = MPCTensor([], precision=precision)
    ...
    def clone(self):
        ...
        result = MPCTensor([], precision=self.encoder._precision_bits)
    ...
    def cuda(self, *args, **kwargs):
        ...
        self.share = self.share.cuda(*args, **kwargs)
        return self
    ...
    def _ltz(self):
        #shift = torch.iinfo(torch.long).bits - 1
        shift = torch.iinfo(self.share.dtype).bits - 1
        #precision = int(math.log2(self.encoder.scale))
        #assert(2 ** precision == self.encoder.scale)
        precision = self.encoder._precision_bits
        #precision = 0 if self.encoder.scale == 1 else None
        """Returns 1 for elements that are < 0 and 0 otherwise"""
        
        # print(f"{cfg.get('functions.compare')}=")
        
        if (
            self.ptype == Ptype.arithmetic 
            and comm.get().get_world_size() == 2
            and "packed" in cfg.get("functions.compare")
        ):
            from CrypTen.crypten.mpc.packed_compare import ltz_fast
            use_cpu = cfg.get_with_default("functions.compare_use_cpu", False)
            msb = cfg.get_with_default("functions.compare_msb", None)
            lsb = cfg.get_with_default("functions.compare_lsb", None)
            # with crypten.LogOperation("A2B"):
            if use_cpu:
                device = self.device
                input = self.to("cpu")
            else:
            ...
            result = ltz_fast(input, msb=msb, lsb=lsb)
            
            # with crypten.LogOperation("B2A"):
            result = result._to_ptype(Ptype.arithmetic, precision=0, bits=1)
